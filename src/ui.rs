//! Обёртка над UEFI SimpleTextOutput/Input.
//! uefi 0.33 API: stdout/stdin через uefi::system::with_stdout / with_stdin.
...
impl Screen {
    ...
    pub fn clear(&self) {
        uefi::system::with_stdout(|out| { out.clear().ok(); });
    }

    pub fn color(&self, (fg, bg): (Color, Color)) {
        uefi::system::with_stdout(|out| { out.set_color(fg, bg).ok(); });
    }

    pub fn print(&self, s: &str) {
        use uefi::CString16;
        if let Ok(cs) = CString16::try_from(s) {
            uefi::system::with_stdout(|out| { out.output_string(&cs).ok(); });
        }
    }
    ...
    pub fn wait_key(&self) {
        self.color(C_DIM);
        self.println("Нажмите любую клавишу...");
        self.color(C_NORMAL);
        loop {
            let key = uefi::system::with_stdin(|inp| inp.read_key().ok().flatten());
    ...
    pub fn read_key(&self) -> Option<Key> {
        uefi::system::with_stdin(|inp| inp.read_key().ok().flatten())
    }

    pub fn stall_ms(&self, ms: usize) {
        uefi::boot::stall(ms * 1_000);
    }
}
